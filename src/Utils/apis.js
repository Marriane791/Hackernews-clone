...
export const getStories = async (type) => {
    try {
        ...
        const stories = await Promise.all (storyIds.slice(0,30).map(getStory));//promise makes al api calls simultenious rather than usng a for loop to fetch each content after the other.
        return stories;
    } catch (error) {
        console.log ('Error while getting a list of stories.');
    }
};